...
var (
	...
	testDirectoryEntries      = []*directoryEntryRaw{
		&directoryEntryRaw{offset: 0x0, inodeNumber: 0x0, inodeType: inodeType(0x1), name: "d1", isSubdirectory: true},
		&directoryEntryRaw{offset: 0x20, inodeNumber: 0x1, inodeType: inodeType(0x1), name: "d2", isSubdirectory: true},
		&directoryEntryRaw{offset: 0x40, inodeNumber: 0x2, inodeType: inodeType(0x1), name: "d3", isSubdirectory: true},
	}
...
func TestParseDirectoryEntry(t *testing.T) {
	...
	for i, tt := range tests {
		entry, err := parseDirectoryEntry(tt.b)
		switch {
		case (err == nil && tt.err != nil) || (err != nil && tt.err == nil) || (err != nil && tt.err != nil && !strings.HasPrefix(err.Error(), tt.err.Error())):
			t.Errorf("%d: mismatched error, actual then expected", i)
			t.Logf("%v", err)
...
func TestParseDirectory(t *testing.T) {
	...
	for i, tt := range tests {
		dir, err := parseDirectory(tt.b, uint32(len(tt.b)))
		switch {
		case (err == nil && tt.err != nil) || (err != nil && tt.err == nil) || (err != nil && tt.err != nil && !strings.HasPrefix(err.Error(), tt.err.Error())):
			t.Errorf("%d: mismatched error, actual then expected", i)
			t.Logf("%v", err)
